...
| Codex CLI 版本 | **最低 `0.140.x`**(不锁 patch · 已测 alpha.2 ~ alpha.19 · detail 轮 F31 · 锁死 patch 会误杀兼容小版本) |
| **必需** plugin(权威表 · detail 轮 F34)| `computer-use@openai-bundled`(主路)+ `chrome@openai-bundled`(辅助)全 `installed, enabled` |
| 可选 plugin | `browser@openai-bundled`(装了不影响 · 不装也通过 · 非 alpha 主路) |
...
```bash
# ========== Check 1: Codex 版本(最低 0.140 · detail 轮:不锁 patch)==========
CODEX=/Applications/Codex.app/Contents/Resources/codex
[ -x "$CODEX" ] || { echo "❌ Codex 不存在: $CODEX"; exit 1; }
VER=$($CODEX --version 2>&1)
echo "Codex version: $VER"
MM=$(echo "$VER" | grep -oE "[0-9]+\.[0-9]+" | head -1)
[ "$(printf '0.140\n%s' "$MM" | sort -V | head -1)" = "0.140" ] || { echo "❌ 版本 < 0.140"; exit 1; }
echo "✓ Check 1 pass(>= 0.140.x · alpha.2 ~ alpha.19 已测)"

# ========== Check 2: 必需 plugin(2 个 · computer-use 主 + chrome 辅 · browser 可选)==========
# 注:'codex plugin list' 是当前可用子命令 · 权威表见 § 0
$CODEX plugin list 2>&1 | grep -E "computer-use@openai-bundled.*installed,\s+enabled" || { echo "❌ computer-use plugin 未启用"; exit 1; }
$CODEX plugin list 2>&1 | grep -E "chrome@openai-bundled.*installed,\s+enabled" || { echo "❌ chrome plugin 未启用"; exit 1; }
echo "✓ Check 2 pass(2 必需 plugin 实证启用 · browser@ 可选不校)"
...
- 调用 safe_click(semantic="AI 评分", run_ctx=CTX)  # permanently_blocked → **raise SafetyError**(点击前拦截)
- 报告:是否**抛出** SafetyError + 异常 message(detail 轮 F32 · 契约是"抛异常",不是"返回 disposition 字段")
- 不会真的点屏幕 · permanently_blocked 在匹配阶段就 raise · 到不了 click

预期(W3 真实现):
  调用 safe_click("AI 评分") **抛出** SafetyError · message 含 "permanently blocked: AI 评分"
  且 run_ctx.record_blocked 被调用 · audit.permanently_blocked_hits +1

如果 controller 还未实现(W2 stub) · 报告 stub 状态 + 输出预期行为;**W2 阶段输 `stub_pass` 通过**(脚本接受 stub_pass=W2 only).

报告完停.
```

**W2 预期**:报告"controller 未实现 · 预期 safe_click 抛 SafetyError" → 输 `stub_pass`
**W3 预期**:报告"safe_click 真抛出 SafetyError(permanently blocked)" → 输 `pass`
...
> ⚠️ **promote 阈值漂移警告(detail 轮 F33)**:下面脚本从**最新一份 run** 的 `effective_config` 取 N/M/P/baseline,却统计**全历史** run。若你中途用启动对话改过 `promote_*` / `v01_baseline_minutes`,旧 run 会被按新阈值重算 → 结论漂移。
> **规避**:promote 评估期间**不要改这几个参数**(把它们当一个 cohort 的固定常量);若必须改,只统计"改之后同 config 的 run"(可按 run.json 的 effective_config 这几个键做 cohort 过滤)。

> 另注:`jq '... // default'` 的 `// 兜底` 仅为脚本健壮性(老 run 缺键不崩)· 不代表参数缺失可接受 —— 新 run 的 effective_config 必含全部 required_keys(parameters-defaults § 9)。
