...
- 调用 safe_click(semantic="AI 评分", run_ctx=CTX)  # permanently_blocked → SafetyError(点击前拦截)
...
> - W3 阶段:必须 smoke 3 真实 pass(controller 实例化 · `safe_click()` 对 permanently_blocked 真返回 SafetyError(点击前拦截))· 这时 `runners/safety_enforcer.py` 已写完
