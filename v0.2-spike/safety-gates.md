...
```python
def safe_click(button_target, run_ctx, pre_signed_token=None):
    """
    α.3.3 r9 闭环 · 审批模型唯一(钉死 · 不再双轨):
      - pre_signed_token != None → runner 已在主流程 § 7 预签 · gates 仅 consume · 不二次 await
      - pre_signed_token == None → 兜底(legacy / 非主流程入口)· gates 自签自 consume
      - 主流程 saveContact 必走"预签",runner 在 SKILL § 4 step 7 已签好
    """
    ...
    if matched_guarded:
        ...
        payload = canonical_payload(button_target)
        # α.3.3 r9 · 审批唯一性:有 pre_signed_token 就只 consume · 无则兜底自签
        if pre_signed_token is not None:
            token = pre_signed_token   # runner 预签 · 不再二次 await
            # token.action_id 必须 == matched_guarded.action_id · token.payload 必须 == payload(consume 内部校验 sig)
        else:
