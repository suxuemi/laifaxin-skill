...
alpha_scope:
  in:
    ...
    - 翻页判精度(LLM 节点 2)
    - 保存联系人(单家最多 emails_per_company 个邮箱 · 整数 · default 5 · range 3-10)
...
def safe_click(button_target, run_ctx, pre_signed_token=None):
    """
    r10 钉死 · 审批模型唯一(单轨 · 无 legacy 自签兜底):
      - 命中 guarded action 时,**必须**带 pre_signed_token(runner 在主流程 § 7 预签)
      - gates 只 consume · 绝不自己 await_user_token(消除双重审批/前层空转)
      - 若 guarded 但 pre_signed_token 缺失 → fail-closed(不自签 · 直接报错让 runner 补签)
      - allowed / permanently_blocked 分支不需要 token(allowed 直点 · blocked 直拒)
    """
